...
_portfolio_column_formatters = {
    ...
    "Average Buy Price": ColumnFormatter(
        ...
        12,
        lambda entry: CellData(
            format_number(entry.average_cost),
            Fore.GREEN if entry.average_cost <= entry.stock.curr_value else Fore.RED,
        ),
    ),
...
class Renderer(metaclass=utils.Singleton):
    ...
    def print_entries(
        self,
        print_cols=list(_stock_column_formatters.keys())
        + list(_portfolio_column_formatters.keys()),
    ):
        # print the heading
        heading = "\n\t"
        divider = "\t"
